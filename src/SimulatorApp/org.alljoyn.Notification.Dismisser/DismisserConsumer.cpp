...
std::map<alljoyn_interfacedescription, WeakReference*> DismisserConsumer::SourceInterfaces;

DismisserConsumer::DismisserConsumer(AllJoynBusAttachment^ busAttachment)
...
DismisserConsumer::~DismisserConsumer()
{
...
void DismisserConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void DismisserConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void DismisserConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus DismisserConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<DismisserJoinSessionResult^>^ DismisserConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ DismisserWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> DismisserJoinSessionResult^
    {
        auto result = ref new DismisserJoinSessionResult();
        result->Consumer = ref new DismisserConsumer(watcher->BusAttachment);
...
IAsyncOperation<DismisserGetVersionResult^>^ DismisserConsumer::GetVersionAsync()
{
    return create_async([this]() -> DismisserGetVersionResult^
    {
        ...
        auto result = ref new DismisserGetVersionResult();
...
void DismisserConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    UNREFERENCED_PARAMETER(obj);
...
void DismisserConsumer::CallDismissSignalHandler(_In_ const alljoyn_interfacedescription_member* member, _In_ alljoyn_message message)
{
    ...
    auto consumer = source->second->Resolve<DismisserConsumer>();
    if (consumer->Signals != nullptr)
    {
        ...
        consumer->Signals->CallDismissReceived(consumer->Signals, eventArgs);
    }
}

int32 DismisserConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<DismisserConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<DismisserConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<DismisserConsumer>
    };
    ...
    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "org.alljoyn.Notification.Dismisser");
