...
std::map<alljoyn_interfacedescription, WeakReference*> ProducerConsumer::SourceInterfaces;

ProducerConsumer::ProducerConsumer(AllJoynBusAttachment^ busAttachment)
...
void ProducerConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void ProducerConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void ProducerConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus ProducerConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<ProducerJoinSessionResult^>^ ProducerConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ ProducerWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> ProducerJoinSessionResult^
    {
        auto result = ref new ProducerJoinSessionResult();
        result->Consumer = ref new ProducerConsumer(watcher->BusAttachment);
        result->Status = result->Consumer->JoinSession(serviceInfo);
        return result;
    });
}

IAsyncOperation<ProducerDismissResult^>^ ProducerConsumer::DismissAsync(_In_ int32 interfaceMemberInputArg)
{
    return create_async([this, interfaceMemberInputArg]() -> ProducerDismissResult^
    {
        ...
        status = static_cast<QStatus>(TypeConversionHelpers::SetAllJoynMessageArg(alljoyn_msgarg_array_element(inputs, 0), "i", interfaceMemberInputArg));
...
IAsyncOperation<ProducerGetVersionResult^>^ ProducerConsumer::GetVersionAsync()
{
    return create_async([this]() -> ProducerGetVersionResult^
    {
...
void ProducerConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    UNREFERENCED_PARAMETER(obj);
...
int32 ProducerConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<ProducerConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<ProducerConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<ProducerConsumer>
    };
    ...
    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "org.alljoyn.Notification.Producer");
