...
ProducerWatcher::ProducerWatcher(AllJoynBusAttachment^ busAttachment) :
...
ProducerWatcher::~ProducerWatcher()
{
    UnregisterFromBus();
}

void ProducerWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.alljoyn.Notification.Producer" };
...
void ProducerWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.alljoyn.Notification.Producer"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.alljoyn.Notification.Producer"),
            port);
        Added(this, args);
    }
...
void ProducerWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<ProducerWatcher>
        };
...
void ProducerWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_ProducerIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &ProducerWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void ProducerWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void ProducerWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
