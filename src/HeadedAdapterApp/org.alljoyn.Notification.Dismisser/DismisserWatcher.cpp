...
DismisserWatcher::DismisserWatcher(AllJoynBusAttachment^ busAttachment) :
...
DismisserWatcher::~DismisserWatcher()
{
    UnregisterFromBus();
}

void DismisserWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.alljoyn.Notification.Dismisser" };
...
void DismisserWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.alljoyn.Notification.Dismisser"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.alljoyn.Notification.Dismisser"),
            port);
        Added(this, args);
    }
...
void DismisserWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<DismisserWatcher>
        };
...
void DismisserWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_DismisserIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &DismisserWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void DismisserWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void DismisserWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
