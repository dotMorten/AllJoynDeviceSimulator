...
std::map<alljoyn_busobject, WeakReference*> DismisserProducer::SourceObjects;
std::map<alljoyn_interfacedescription, WeakReference*> DismisserProducer::SourceInterfaces;

DismisserProducer::DismisserProducer(AllJoynBusAttachment^ busAttachment)
    ...
    m_sessionId(0)
{
    ...
    m_signals = ref new DismisserSignals();
    m_busAttachmentStateChangedToken.Value = 0;
}

DismisserProducer::~DismisserProducer()
{
    UnregisterFromBus();
    delete m_weak;
}

void DismisserProducer::UnregisterFromBus()
{
...
bool DismisserProducer::OnAcceptSessionJoiner(_In_ alljoyn_sessionport sessionPort, _In_ PCSTR joiner, _In_ const alljoyn_sessionopts opts)
{
...
void DismisserProducer::OnSessionJoined(_In_ alljoyn_sessionport sessionPort, _In_ alljoyn_sessionid id, _In_ PCSTR joiner)
{
    ...
    {
        AllJoynHelpers::SessionLostHandler<DismisserProducer>,
        AllJoynHelpers::SessionMemberAddedHandler<DismisserProducer>,
        AllJoynHelpers::SessionMemberRemovedHandler<DismisserProducer>
    };
...
void DismisserProducer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void DismisserProducer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void DismisserProducer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void DismisserProducer::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {   
        QStatus result = AllJoynHelpers::CreateProducerSession<DismisserProducer>(m_busAttachment, m_weak);
        if (ER_OK != result)
        {
            StopInternal(result);
            return;
...
void DismisserProducer::CallDismissSignalHandler(_In_ const alljoyn_interfacedescription_member* member, _In_ alljoyn_message message)
{
    ...
    auto producer = source->second->Resolve<DismisserProducer>();
    if (producer->Signals != nullptr)
    {
        ...
        auto eventArgs = ref new DismisserDismissReceivedEventArgs();
        ...
        Windows::Foundation::Collections::IVector<byte>^ argument1;
        (void)TypeConversionHelpers::GetAllJoynMessageArg(alljoyn_message_getarg(message, 1), "ay", &argument1);
        eventArgs->Arg2 = argument1;

        producer->Signals->CallDismissReceived(producer->Signals, eventArgs);
    }
}

QStatus DismisserProducer::AddMethodHandler(_In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_methodhandler_ptr handler)
{
...
QStatus DismisserProducer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
    alljoyn_interfacedescription_member member;
    if (!alljoyn_interfacedescription_getmember(interfaceDescription, methodName, &member))
    {
...
QStatus DismisserProducer::OnPropertyGet(_In_ PCSTR interfaceName, _In_ PCSTR propertyName, _Inout_ alljoyn_msgarg value)
{
...
QStatus DismisserProducer::OnPropertySet(_In_ PCSTR interfaceName, _In_ PCSTR propertyName, _In_ alljoyn_msgarg value)
{
    UNREFERENCED_PARAMETER(interfaceName);
...
void DismisserProducer::Start()
{
    ...
    QStatus result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_DismisserIntrospectionXml);
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    result = AllJoynHelpers::CreateBusObject<DismisserProducer>(m_weak);
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    alljoyn_interfacedescription interfaceDescription = alljoyn_busattachment_getinterface(AllJoynHelpers::GetInternalBusAttachment(m_busAttachment), "org.alljoyn.Notification.Dismisser");
    ...
    result = AddSignalHandler(
        ...
        "Dismiss",
        [](const alljoyn_interfacedescription_member* member, PCSTR srcPath, alljoyn_message message) { UNREFERENCED_PARAMETER(srcPath); CallDismissSignalHandler(member, message); });
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^,AllJoynBusAttachmentStateChangedEventArgs^>(this, &DismisserProducer::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void DismisserProducer::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void DismisserProducer::StopInternal(int32 status)
{
    UnregisterFromBus();
    Stopped(this, ref new AllJoynProducerStoppedEventArgs(status));
}

int32 DismisserProducer::RemoveMemberFromSession(_In_ String^ uniqueName)
{
...
PCSTR org::alljoyn::Notification::Dismisser::c_DismisserIntrospectionXml = "<interface name=\"org.alljoyn.Notification.Dismisser\">"
"  <signal name=\"Dismiss\">"
"    <arg type=\"i\" />"
"    <arg type=\"ay\" />"
"  </signal>"
